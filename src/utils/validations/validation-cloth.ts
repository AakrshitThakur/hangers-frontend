import type { CreateCloth, UpdateCloth } from "../../types/clothes.types";
...
function validateUpdateCloth(input: UpdateCloth) {
  // validation errors
  const errors = {
    title: "",
    clothImages: "",
  ...
  if (input.clothImages.length + input.publicIds.length >= 4) {
    errors.clothImages = "Maximum of three images allowed";
    errors.hasErrors = true;
  }
  //   if (!TOP_3_CLOTH_VALUES.includes(`${input.isTop3}`)) {
...
export { validateCreateCloth, validateUpdateCloth };
